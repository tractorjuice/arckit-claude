...
2. **Read Available Documents**:
   ...
   **MANDATORY** (warn if missing):
   - **AAGI** (Agent Inventory) — Extract: Agent list, capabilities, risk levels
     - If missing: warn user to run `/arckit-agent-architecture:agent-inventory` first
   - **AAGR** (Agent Design) — Extract: Design patterns, decision-making autonomy, integration points
     - If missing: warn user to run `/arckit-agent-architecture:agent-design` first
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/arckit-agent-architecture:agent-governance` command | [PENDING] | [PENDING] |
```
...
```markdown
**Generated by**: ArcKit `/arckit-agent-architecture:agent-governance` command
...
User: `/arckit-agent-architecture:agent-governance research-agent`
...
- Governance framework feeds into agent security requirements — handoff to `/arckit-agent-architecture:agent-security` after completion
