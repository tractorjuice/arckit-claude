...
- **ADMP** (Architecture Vision / Preliminary ADM) — Extract: Scope boundaries, ADM phase inclusions, strategic vision
  - If missing: STOP and ask user to run `/arckit-togaf-adm:adm-preliminary` first. The application inventory must be scoped by the architecture vision.
...
```markdown
| 1.0 | {DATE} | ArcKit AI | Initial creation from `/arckit-togaf-adm:application-inventory` command |
```
...
2. Run `/arckit-togaf-adm:application-rationalization` to decide keep/merge/replace/retire
3. Run `/arckit-togaf-adm:gap-analysis` to identify capability coverage gaps
...
6. **Integration with Other Commands**:
   ...
   - **Output**: Feeds `/arckit-togaf-adm:application-rationalization` (portfolio decisions), `/arckit-togaf-adm:gap-analysis` (capability gaps)
